...
function SlotDigit({ digit, delay }: { digit: string; delay: number }) {
  ...
  return (
    <span
      className="slot-reel"
      style={{ '--slot-distance': `${target}em`, '--slot-delay': `${delay}ms` } as React.CSSProperties}
    >
...
function AnimatedScore({ score }: { score: number }) {
  ...
  if (!show) return <span className="text-7xl font-black text-brand-600 dark:text-brand-400">0</span>;
  ...
  return (
    <div className="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden shadow-inner">
      <div
        ...
        style={{
          width: started ? `${pct}%` : '0%',
          transition: started ? 'width 0.9s cubic-bezier(0.25, 0.46, 0.45, 0.94)' : 'none',
        }}
      />
    </div>
  );
...
function ResultScreen({
  score, questions, answers, onHome,
}: {
  score: number;
  questions: Question[];
  answers: number[];
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-12 px-6 animate-fade-in">
      {/* 트로피 */}
      <div className={`w-24 h-24 rounded-[2.5rem] flex items-center justify-center text-5xl mb-6 shadow-inner ${
        pass ? 'bg-brand-50 dark:bg-brand-900/30' : 'bg-rose-50 dark:bg-rose-900/30'
      }`}>
        {pass ? '🏆' : '📖'}
      </div>

      <h2 className={`text-2xl font-black mb-6 ${pass ? 'text-brand-600' : 'text-rose-500'}`}>
        {pass ? '합격입니다!' : '불합격입니다.'}
      </h2>
      ...
      <div className="mb-1 flex items-end gap-2">
        ...
        <span className="text-4xl font-black text-brand-600 dark:text-brand-400 mb-1">점</span>
      </div>
          ...
          return (
            <div key={cat}>
              <div className="flex justify-between text-xs font-bold text-slate-500 mb-2">
                <span>{cat}</span>
                <span className="text-brand-500">{pct}% ({stat.correct}/{stat.total})</span>
              </div>
      ...
      <button onClick={onHome} className="btn-primary max-w-xs w-full">홈으로</button>
    </div>
  );
}
...
function QuizContent() {
  ...
  const score = questions.length > 0
    ? Math.round((answers.filter((a, i) => a === questions[i]?.answer_index).length / questions.length) * 100)
    : 0;
  ...
  useEffect(() => {
    async function load() {
      ...
      if (examType === 'wrong') {
        ...
        qs = (data || []).map((r: any) => r.question as Question);
      }
  ...
  async function handleAnswer(idx: number) {
    ...
    if (isCorrect) {
      await supabase.from('wrong_answers').delete()
        .eq('student_id', student.id).eq('question_id', q.id);
    } else {
      await supabase.from('wrong_answers')
        .upsert({ student_id: student.id, question_id: q.id }, { onConflict: 'student_id,question_id' });
    }
  }
  ...
  if (phase === 'loading') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-400 font-semibold text-sm">문제를 불러오는 중...</p>
      </div>
    );
  }

  if (phase === 'result') {
    return <ResultScreen score={score} questions={questions} answers={answers} onHome={() => router.push('/home')} />;
  }
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen flex flex-col p-5">
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => router.push('/home')} className="p-2 text-slate-400 active:scale-90 transition-all">
          <ArrowLeft size={22} />
        </button>
        ...
        <span className={`text-2xl font-black text-rose-500 ${timerBlink}`}>{timeLeft}</span>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="flex justify-between text-xs font-black text-slate-400 mt-2 tracking-widest">
          <span>PROGRESS</span>
          <span><span className="text-brand-500">{currentIdx + 1}</span> / {questions.length}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-4">
        ...
        {q.image_url && (
          <figure className="mb-6 rounded-2xl overflow-hidden border-4 border-slate-100 dark:border-slate-800 shadow-md">
            <img src={q.image_url} alt="문제 이미지" className="w-full h-auto" />
          </figure>
        )}

        <div className="space-y-3">
          {q.options.map((opt, i) => {
            let cls = 'w-full text-left p-5 rounded-2xl border-2 transition-all flex items-center gap-4 ';
            let badgeCls = 'flex-none w-8 h-8 rounded-full text-sm font-black flex items-center justify-center ';

            if (!answered) {
              cls += 'border-slate-100 dark:border-slate-800 active:bg-slate-50 dark:active:bg-slate-900 shadow-sm';
            ...
            return (
              <button key={i} onClick={() => !answered && handleAnswer(i)} disabled={answered} className={cls}>
                <span className={badgeCls}>{i + 1}</span>
                <span className="dark:text-slate-200 font-semibold text-sm leading-snug">{opt}</span>
              </button>
            );
          })}
        </div>
      ...
      {feedbackOpen && (
        <div className="fixed inset-x-0 bottom-0 z-50 animate-slide-up">
          <div className="max-w-md mx-auto px-4 pb-4">
            <div className={`rounded-3xl p-6 shadow-2xl space-y-4 ${isCorrect ? 'bg-emerald-600' : 'bg-rose-600'}`}>
              <p className="text-base font-black text-yellow-300">
                {isCorrect ? '정답입니다! ✅' : `오답! 정답은 ${q.answer_index + 1}번입니다. ❌`}
              </p>
              {q.explanation && (
