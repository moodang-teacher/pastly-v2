...
import { LogOut, Sun, Moon, Trophy, ChevronDown, Camera } from 'lucide-react';
...
const EXAM_TYPES = [
  { value: 'past_exam', label: '📚 기출문제 풀이', desc: '실제 기출문제 랜덤 60문항' },
  ...
  const loadData = useCallback(async () => {
    ...
    if (!st) {
      const { data: newSt } = await supabase
        .from('students')
        .insert({ user_id: user.id, name: user.email?.split('@')[0] || '학생', total_attempts: 0, high_score: 0 })
        .select()
        .single();
      setStudent(newSt);
      setNoDept(true);
    ...
    const { data: rank } = await supabase.rpc('get_rankings', { p_department_id: st.department_id });
    setRankings(rank || []);
    ...
    <div className="max-w-md mx-auto min-h-screen flex flex-col p-5 pb-8">
      ...
      <header className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={toggleTheme} className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 transition-all active:scale-90">
            {isDark ? <Sun size={18} /> : <Moon size={18} />}
          </button>
          <button onClick={handleLogout} className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-rose-500 transition-all active:scale-90">
            <LogOut size={18} />
          </button>
        </div>
      </header>
      ...
      <div className="card p-5 flex items-center gap-4 mb-5">
        ...
        <div className="relative flex-none">
          ...
          >
            {student?.photo_url
              ? <img src={student.photo_url} className="w-full h-full object-cover" alt="아바타" />
              : <span>👤</span>
            }
            {/* 호버 오버레이 */}
        ...
        <div className="flex-1 min-w-0">
          <p className="font-black text-lg truncate dark:text-white">{student?.name}</p>
          <p className="text-xs text-brand-500 font-bold">{getLevelLabel(student?.total_attempts || 0)}</p>
          {department && <p className="text-xs text-slate-400 font-medium mt-0.5">{department.name}</p>}
        </div>
        {noDept && (
      ...
      <div className="relative mb-4">
        ...
        >
          ...
          <ChevronDown size={18} className={`text-slate-400 transition-transform ${showExamDrop ? 'rotate-180' : ''}`} />
        </button>
        {showExamDrop && (
          <div className="absolute top-full left-0 right-0 mt-2 card shadow-xl z-20 overflow-hidden animate-pop">
            {EXAM_TYPES.map(et => (
              <button
                key={et.value}
                onClick={() => { setSelectedExam(et.value); setShowExamDrop(false); }}
                className={`w-full p-4 text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b last:border-0 border-slate-100 dark:border-slate-700 ${
                  selectedExam === et.value ? 'bg-brand-50 dark:bg-brand-900/20' : ''
                }`}
              >
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        ...
        >
          ...
          <span className="text-brand-800 dark:text-brand-300 font-black text-sm tracking-tight">
            {EXAM_TYPES.find(e => e.value === selectedExam)?.label.split(' ').slice(1).join(' ')}
          </span>
        </button>
        ...
        >
          ...
          <span className="text-rose-800 dark:text-rose-300 font-black text-sm tracking-tight">오답 복습</span>
          {wrongCount > 0 && (
      ...
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-black text-slate-800 dark:text-slate-100 tracking-tighter">명예의 전당</h3>
          {department && (
        ...
        {noDept ? (
          <div className="card p-8 text-center text-slate-400">
            ...
            <p className="font-semibold text-sm">전공 배정 후 랭킹을 볼 수 있어요</p>
          </div>
        ) : rankings.length === 0 ? (
          <div className="card p-8 text-center text-slate-400">
            <p className="font-semibold text-sm">아직 기록이 없어요. 첫 번째 도전자가 되세요!</p>
          </div>
        ) : (
              ...
              <div key={i} className="card p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-brand-400 font-medium">{getLevelLabel(r.total_attempts || 0)}</p>
                  </div>
                </div>
                <span className="font-black text-brand-600 dark:text-brand-400">{r.high_score}점</span>
              </div>
            ))}
          </div>
        )}
