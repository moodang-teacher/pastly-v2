...
import { Upload, Trash2, Eye, EyeOff, AlertTriangle, CheckCircle, PenLine, ImagePlus, X } from 'lucide-react';
...
export default function QuestionsPage() {
  ...
  const [sExamType, setSExamType] = useState<ExamType>('past_exam');
  ...
  useEffect(() => {
    async function load() {
      ...
      const filtered = (depts || []).filter((d: any) => {
        ...
        if (myDept.parent_id === null && myDept.slug !== 'beauty') return d.id === myDept.id;
      ...
      const { data: qs } = await supabase
        .from('questions').select('*').eq('uploaded_by', t?.id)
  ...
  async function handleJsonUpload() {
    ...
    if (error) {
      ...
      const { data: qs } = await supabase.from('questions').select('*').eq('uploaded_by', teacher?.id)
        .order('created_at', { ascending: false }).limit(50);
      setQuestions(qs || []);
    }
    setUploading(false);
  ...
  async function uploadImageToStorage(file: File, teacherId: string): Promise<string> {
  ...
  async function handleSingleSave() {
    ...
    if (sOptions.some(o => !o.trim())) return setSSaveResult('❌ 선택지 4개를 모두 입력해주세요.');
    ...
    try {
      ...
      // 폼 초기화
      setSCategory(''); setSQuestion(''); setSOptions(['', '', '', '']);
      setSAnswer(null); setSExplanation(''); setSImageFile(null); setSImagePreview('');
      if (imgFileRef.current) imgFileRef.current.value = '';
      ...
      {activeTab === 'json' && (
        <div className="card p-5 space-y-4">
          ...
          <div>
            ...
            <select value={uploadDept} onChange={e => setUploadDept(e.target.value)} className="input-field">
              ...
              {(Object.keys(EXAM_TYPE_LABELS) as ExamType[]).map(t => (
                <button key={t} onClick={() => setUploadType(t)}
                  className={`py-2.5 px-3 rounded-xl text-xs font-bold transition-all ${
                    uploadType === t ? 'bg-brand-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'
                  }`}
                >
                  {EXAM_TYPE_LABELS[t]}
                </button>
          ...
          {teacher?.department?.slug?.startsWith('beauty-') && (
            <label className="flex items-center gap-3 cursor-pointer">
              <div onClick={() => setIsCommon(!isCommon)}
              ...
              <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">미용 공통 과목</span>
            </label>
          )}

          <div onClick={() => jsonFileRef.current?.click()}
            ...
            <p className="text-sm font-semibold text-slate-500">{fileName || 'JSON 파일을 선택하세요'}</p>
          ...
          {uploadResult && (
            <p className={`text-sm font-bold text-center py-3 rounded-2xl ${
              uploadResult.startsWith('✅') ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-500'
            }`}>{uploadResult}</p>
          )}

          <button onClick={handleJsonUpload} disabled={!parsed || uploading}
          ...
          <details className="group">
            <summary className="text-xs font-bold text-slate-400 cursor-pointer hover:text-slate-600 list-none flex items-center gap-1">
              <span className="group-open:rotate-90 transition-transform inline-block">▶</span> JSON 형식 가이드 보기
            </summary>
  ...
  "questions": [
    {
      ...
      {activeTab === 'single' && (
        <div className="card p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-slate-500 mb-1.5 block">전공</label>
              <select value={sDept} onChange={e => setSDept(e.target.value)} className="input-field">
                {departments.map((d: any) => <option key={d.id} value={d.id}>{d.name}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 mb-1.5 block">유형</label>
              <select value={sExamType} onChange={e => setSExamType(e.target.value as ExamType)} className="input-field">
                {(Object.keys(EXAM_TYPE_LABELS) as ExamType[]).map(t => (
          ...
          {teacher?.department?.slug?.startsWith('beauty-') && (
            <label className="flex items-center gap-3 cursor-pointer">
              <div onClick={() => setSIsCommon(!sIsCommon)}
                className={`w-12 h-6 rounded-full transition-colors ${sIsCommon ? 'bg-brand-500' : 'bg-slate-200 dark:bg-slate-700'}`}>
                <div className={`w-5 h-5 bg-white rounded-full shadow transition-transform m-0.5 ${sIsCommon ? 'translate-x-6' : ''}`} />
              </div>
              <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">미용 공통 과목</span>
            </label>
          )}
          ...
          {/* 카테고리 */}
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1.5 block">카테고리 (과목명)</label>
            <input className="input-field" placeholder="예: 헤어미용, 공중위생관리학"
              value={sCategory} onChange={e => setSCategory(e.target.value)} />
          </div>

          {/* 문제 */}
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1.5 block">문제</label>
            <textarea className="input-field resize-none" rows={3}
              placeholder="문제 내용을 입력하세요"
              value={sQuestion} onChange={e => setSQuestion(e.target.value)} />
          </div>

          {/* 이미지 첨부 */}
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1.5 block">이미지 첨부 (선택)</label>
            {sImagePreview ? (
              <div className="relative">
                <img src={sImagePreview} alt="미리보기" className="w-full rounded-2xl object-cover max-h-48" />
              ...
              <div onClick={() => imgFileRef.current?.click()}
                ...
                <p className="text-sm text-slate-400 font-semibold">이미지 선택</p>
                <p className="text-xs text-slate-300 mt-0.5">JPG, PNG 등</p>
                <input ref={imgFileRef} type="file" accept="image/*" onChange={handleImageSelect} className="hidden" />
              </div>
            )}
          </div>
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1.5 block">
              선택지 <span className="text-slate-400 font-normal">(정답 번호 클릭으로 선택)</span>
            </label>
            ...
            {sAnswer !== null && (
              <p className="text-xs text-emerald-500 font-bold mt-2 px-1">✓ {sAnswer + 1}번이 정답으로 설정됨</p>
            )}
          </div>

          {/* 해설 */}
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1.5 block">해설 (선택)</label>
            <textarea className="input-field resize-none" rows={2}
              placeholder="해설을 입력하세요 (없어도 됩니다)"
              value={sExplanation} onChange={e => setSExplanation(e.target.value)} />
          </div>

          {sSaveResult && (
            <p className={`text-sm font-bold text-center py-3 rounded-2xl ${
              sSaveResult.startsWith('✅') ? 'bg-emerald-50 text-emerald-600'
              : sSaveResult.startsWith('🔄') ? 'bg-blue-50 text-blue-600'
              : 'bg-rose-50 text-rose-500'
            }`}>{sSaveResult}</p>
          )}

          <button onClick={handleSingleSave} disabled={sSaving}
      ...
      <div className="card p-5">
        <h2 className="font-black text-slate-800 dark:text-white mb-4">내가 올린 문제 ({questions.length})</h2>
        {questions.length === 0 ? (
          <p className="text-center text-slate-400 py-6 text-sm">아직 올린 문제가 없습니다</p>
        ) : (
          <div className="space-y-2 max-h-96 overflow-y-auto no-scrollbar">
            {questions.map(q => (
              <div key={q.id} className={`p-3 rounded-xl border flex items-start gap-3 ${
                q.is_active ? 'border-slate-100 dark:border-slate-700' : 'border-rose-100 dark:border-rose-900/30 opacity-50'
              }`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    ...
                    {q.image_url && <span className="text-xs text-amber-500">🖼 이미지</span>}
                  </div>
