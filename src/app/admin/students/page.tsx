...
export default function StudentsPage() {
  ...
  async function assignAll() {
    ...
    if (
      !confirm(
        `${waitingStudents.length}명 전체를 선택한 기수에 배정하시겠습니까?`,
      )
    )
      return;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {msg && (
        <p className="text-center text-sm font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 py-2.5 rounded-2xl">
          {msg}
        </p>
      )}
      ...
      {waitingStudents.length > 0 && (
        <div className="card p-5">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                checked={
                  waitingStudents.length > 0 &&
                  checkedIds.size === waitingStudents.length
                }
          ...
          {cohorts.length === 0 ? (
            <div className="bg-amber-50 dark:bg-amber-950 p-4 rounded-xl mb-3 flex items-center justify-between gap-3">
              <p className="text-xs text-amber-700 dark:text-amber-300 font-semibold">
                ⚠️ 기수를 먼저 생성해야 배정할 수 있습니다.
              </p>
              <a
                href="/admin/cohorts"
                className="flex-none text-xs font-black px-3 py-1.5 bg-amber-500 text-white rounded-xl active:scale-95 transition-all whitespace-nowrap"
              >
                기수 만들기 →
              </a>
            </div>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        setChangingDeptId(changingDeptId === s.id ? null : s.id)
                      }
                ...
                {changingDeptId === s.id && (
                  <div className="mt-2 flex gap-2">
                    <select
                      ...
                      defaultValue=""
                      onChange={(e) =>
                        e.target.value && changeDept(s.id, e.target.value)
                      }
                    >
                      <option value="">다른 전공 선택...</option>
                      {allDepts
                        .filter((d) => d.id !== teacher?.department_id)
                        .map((d: any) => (
