...
const NAV = [
	{ href: '/master', icon: LayoutDashboard, label: '전체 현황' },
	{ href: '/master/students', icon: Users, label: '전체 학생' },
];

export default function MasterNav({ name }: { name: string }) {
		...
		<header className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-40">
			<div className="max-w-2xl mx-auto px-5 py-3 flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					<span className="text-xs bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded-lg font-bold">
						마스터
					</span>
				</div>
			...
			<nav className="max-w-2xl mx-auto px-5 flex gap-1 overflow-x-auto no-scrollbar pb-1">
				{NAV.map((n) => {
					const active = path === n.href || (n.href !== '/master' && path.startsWith(n.href));
					return (
						<Link
							key={n.href}
							href={n.href}
